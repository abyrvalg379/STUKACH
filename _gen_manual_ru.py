...
p(doc, '42 чека в 7 категориях + Scene Units. Состав и критичность соответствуют коду v1.8.2; '
       'пороги части чеков настраиваются в Preferences (вкладка UV, вкладка Checks - порог '
       'фасок для Hard Edges). Проверка обновлений - §9.1.')
...
add_table(doc, [
    ...
    ['Ngons', 'BLOCKER', 'Грани с 5+ рёбрами',
     'В пайплайне не допускаются. Фикс в один клик: триангуляция'],
    ...
    ['Sharp Edges Not Hard', 'WARNING', 'Гладкие рёбра с углом ≥ 30° между большими гранями, '
     'не помеченные Sharp',
     'Гладкое затенение на остром угле даёт артефакт шейдинга. Скипается: меши с кастомными '
     'нормалями, фаски-полоски (порог в Preferences - Checks), пары flat-граней'],
...
p(doc, 'Preferences → Add-ons → STUKACH. Вкладки: Interface, Overlay, UV, Naming, Checks, Colors.')
...
add_table(doc, [
    ...
    ['Naming', 'Objects и Groups: списки префиксов/суффиксов (+/−); Mesh Data: суффикс дата-блока '
     ...
     '24 встроенных слоя).'],
    ['Checks', 'Hard Edges: порог ширины фаски (Chamfer Width). Ребро, примыкающее к полосе тоньше '
     'этого процента от размера объекта, считается фаской и не флагается.'],
    ['Colors', 'Цвет оверлея каждого чека, сетка в две колонки; применяется к вьюпорту сразу.'],
], [3, 16])

h2(doc, '9.1 Обновления')
p(doc, 'Блок Updates: кнопка Check for updates сравнивает установленную версию с последним '
       'релизом на GitHub (один анонимный запрос, интерфейс не блокируется).')
for b in [
    'Галка Check for updates daily (включена по умолчанию) - тихая автопроверка раз в сутки; '
    'если новой версии нет, ничего нигде не появляется.',
    'При доступном обновлении: строка Update available: vX.Y.Z в префах становится ссылкой на '
    'страницу релиза, и бейдж появляется вверху панели STUKACH.',
    'Обновление ставится штатно: скачать zip со страницы релиза и установить через '
    'Install from Disk (§2); после установки бейдж гаснет.',
]:
    p(doc, b, bullet=True)
