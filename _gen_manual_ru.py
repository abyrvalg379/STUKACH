...
def add_table(doc, rows, col_widths_cm, sev_col=None):
    ...
    for r_idx, row_data in enumerate(rows):
        row = table.add_row()
        # cantSplit + tblHeader. Маленьким таблицам (<=2 рядов) не даём —
        # LibreOffice теряет cantSplit-строку, не влезающую в остаток страницы.
        # Вместо этого клеим их в один блок через keepNext (шапка-сирота недопустима).
        big = len(rows) > 2
        if big:
...
def add_toc_field(doc):
    ...
    hrun = hint.add_run('Если номера страниц в оглавлении не заполнились — выделите оглавление и нажмите F9 (или правый клик → «Обновить поле»).')
    hrun.font.name = 'Arial'
    hrun.font.size = Pt(8.5)
    hrun.font.italic = True
    hrun.font.color.rgb = GREY
...
p(doc, 'STUKACH проверяет сцену на типовые ошибки пайплайна: топологию, трансформы, '
       ...
       'и чинит его до сдачи; куратор получает формальный вердикт одним нажатием. '
       'Документ описывает работу с обеими версиями: основная часть — про Blender-версию, '
       'раздел 11 — про Maya.', grey=False)

kv_note(doc, 'Автор: Maksim Kovalev (VVERH). Лицензия GPL-3.0. '
...
for b in [
    ...
    'режим куратора (Coordinator Mode): только блокеры и предупреждения, формальный вердикт;',
...
h2(doc, '1.3 Роли: артист, куратор, лид')
p(doc, 'Отчёт STUKACH — инструмент общения внутри команды, а не формальность. Модель ролей:')
add_table(doc, [
    ['Роль', 'Как работает со STUKACH'],
    ['Артист', 'Работает в Artist Mode с включённым Live: чинит BLOCKER по мере работы, использует '
               'Next Issue для навигации по браку, Fix — для типовых проблем. Сдаёт ассет, когда '
               'блокеры закрыты, а предупреждения осознаны.'],
    ['Куратор', 'Принимает сданное: открывает Coordinator Mode (мусорные INFO скрыты), прогоняет '
                'валидацию, смотрит вердикт. Отчёт кнопкой Copy Report уходит в задачу Cerebro / '
                'мессенджер. При возврате на доработку отчёт служит списком правок.'],
    ['Лид', 'Смотрит финальный вердикт (READY / REVIEW / BLOCKED) и историю отчётов. Может включить '
            'Start in Coordinator Mode, чтобы панель всегда открывалась в режиме приёмки.'],
], [3, 16])
...
kv_note(doc, 'Нумерация версий Blender и Maya независима. В пайплайне VVERH ассет рождается в Maya '
             '(моделинг + иерархия локаторами), через FBX-мост PROKLADKA попадает в Blender, где '
             'проходит приёмку STUKACH. Maya-версия закрывает валидацию на этапе моделинга.')
...
add_table(doc, [
    ...
    ['Coordinator Mode ⇄ Artist Mode', 'Переключение режима куратора/артиста. В Coordinator Mode '
...
p(doc, 'Иерархия ассета рождается в Maya (локаторы) и через FBX-мост приезжает в Blender группами. '
...
p(doc, 'Пресет = набор включённых чеков + правила нейминга. Сохранение — значок + рядом с дропдауном, '
       'удаление — −. Экспорт/импорт файлом — раздача команде: куратор задаёт студийный пресет '
       '(например «midpoly» или «hero subdiv»), артисты его импортируют. Пресет хранится в префах '
       'Blender и переживает обновления аддона.')

h2(doc, '8.3 Отчёты')
add_table(doc, [
    ['Формат', 'Что внутри', 'Куда'],
    ['Copy Summary (артист)', 'Компактная сводка по категориям + подпись валидатора и дата', 'Буфер обмена'],
    ['Copy Report (куратор)', 'Вердикт VALIDATION: READY/REVIEW/BLOCKED + секции BLOCKERS (fix first) '
     'и WARNINGS — паста в задачу', 'Буфер обмена'],
...
p(doc, 'Чеки с автоматическим исправлением: Non Applied Transform, Scale (Apply Transform), Modifier '
       ...
       'В Coordinator Mode с включённым Coordinator Lock все Fix скрыты — куратор смотрит, но не чинит.')
...
add_table(doc, [
    ...
    ['Interface', 'Start in Coordinator Mode — панель открывается сразу в режиме приёмки; '
     'Coordinator Lock — прятать все кнопки Fix в Coordinator Mode (куратор только валидирует); '
...
h1(doc, '10. Типовые сценарии')

h2(doc, '10.1 Моделлер ведёт ассет')
for b in [
    'Загрузите студийный пресет (Presets → импорт), включите Live.',
    'RUN по выделению; работайте по списку Objects: Sel → правка → (автопересчёт).',
    'BLOCKER — чинить обязательно, WARNING — осознать, INFO — по вкусу.',
    'Next Issue — быстрый цикл навигации по оставшемуся браку.',
    'Перед сдачей: Coordinator Mode → убедиться, что вердикт не BLOCKED.',
]:
    p(doc, b, bullet=True)

h2(doc, '10.2 Приёмка ассета из Maya')
for b in [
    'В Maya ассет собирается локаторами иерархии (конвенции §6.1); FBX экспортируется мостом PROKLADKA.',
    'Импортируйте FBX в Blender. Live подхватит новые объекты, иерархия пересканируется сама — '
    'результат скана готов через пару секунд после импорта.',
    'Прогоните Run по коллекции ассета; чините структуру фиксами: Add _grp, Renumber, Adopt Orphans.',
    'Сверьте вердикт иерархии в Coordinator Mode — это приёмка структуры, рождённой в Maya.',
]:
    p(doc, b, bullet=True)

h2(doc, '10.3 Куратор принимает работу')
for b in [
    'Откройте Coordinator Mode, прогоните Scene.',
    'Смотрите вердикт: BLOCKED — вернуть артисту с Copy Report в задачу; REVIEW — пройтись по '
    'предупреждениям; READY — в паблиш.',
    'Coordinator Lock (Preferences) — если куратору не нужно чинить руками.',
    'Кнопка Debug Info у артиста — приложить к спорным кейсам.',
]:
    p(doc, b, bullet=True)

h2(doc, '10.4 Паблиш')
for b in [
    'Pre-flight FBX / USD — гейт: блокеры не дадут экспортировать битый ассет.',
    'Checkpoint Save перед экспортом — страховка для возврата.',
    'HTML-отчёт — приложение к задаче паблиша (секция Fix first для доработки).',
]:
    p(doc, b, bullet=True)
...
p(doc, 'Кнопка STUKACH на полке открывает панель с той же логикой, что и в Blender: Run, тулбар '
       'режимов (Coordinator / Live / меню «…» с флагами станции), score-блок, Next Issue, Copy '
...
h2(doc, '11.5 Иерархия локаторов и PROKLADKA')
for b in [
    'Иерархия ассета рождается в Maya: локаторы-группы с конвенциями §6.1 (_grp, base_01, строчные).',
    'FBX-мост PROKLADKA (Blender ↔ Maya ↔ Houdini) переносит структуру в Blender, где валидатор '
    'иерархии делает приёмку: чек-лист «локатор → _grp» закрыт — 0 потерь при транспорте.',
    'Maya-стороны валидации иерархии нет — не дублируйте её вручную.',
]:
    p(doc, b, bullet=True)
